...
/* Grab options */
$arr_args = getopt('', ['host:', 'class:', 'test:', 'nocolors', 'user:', 'auth:']);
...
if ($str_class == 'redis') {
    ...
    exit(TestSuite::run("Redis_Test", $str_filter, $str_host, $auth));
    ...
    exit(TestSuite::run("Redis_Cluster_Test", $str_filter, $str_host, $auth));
} else {
    echo TestSuite::make_bold("RedisSentinel") . "\n";
    exit(TestSuite::run("Redis_Sentinel_Test", $str_filter, $str_host, $auth));
}
?>

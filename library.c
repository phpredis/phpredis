...
redis_sock_read_multibulk_reply_zval(INTERNAL_FUNCTION_PARAMETERS,
    ...
    array_init(z_tab);
    redis_mbulk_reply_loop(INTERNAL_FUNCTION_PARAM_PASSTHRU, redis_sock, z_tab, numElems, UNSERIALIZE_ALL);
}
                               ...
                               TSRMLS_DC)
{
    ...
    while(elements > 0) {
        ...
        switch(reply_type) {
            case TYPE_ERR:
            case TYPE_LINE:
                redis_read_variant_line(redis_sock, reply_type, &z_subelem_p
                    TSRMLS_CC);
                add_next_index_zval(*z_ret, z_subelem_p);
                break;
            case TYPE_INT:
                // Add our long value
            ...
            case TYPE_BULK:
                ...
                redis_read_variant_bulk(redis_sock, reply_info, &z_subelem_p
                    TSRMLS_CC);
                add_next_index_zval(*z_ret, z_subelem_p);
                break;
            case TYPE_MULTIBULK:
                         ...
                         zval *z_tab, void *ctx)
{
	...
	switch(reply_type) {
		case TYPE_ERR:
		case TYPE_LINE:
			redis_read_variant_line(redis_sock, reply_type, &z_ret_p TSRMLS_CC);
			break;
		case TYPE_INT:
			ZVAL_LONG(z_ret_p, reply_info);
			break;
		case TYPE_BULK:
			redis_read_variant_bulk(redis_sock, reply_info, &z_ret_p TSRMLS_CC);
			break;
		case TYPE_MULTIBULK:
	...
	IF_MULTI_OR_PIPELINE() {
		add_next_index_zval(z_tab, z_ret_p);
	} else {
